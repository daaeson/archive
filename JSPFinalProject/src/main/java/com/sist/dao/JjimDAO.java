...
public class JjimDAO {
  ...
  public void jjimInsert(JjimVO vo)
  {
	  try
	  {
		  ...
		  String sql="INSERT INTO project_all_jjim VALUES("
				    +"(SELECT NVL(MAX(jno)+1,1) FROM project_all_jjim),?,?)";
  ...
  */
  ...
  public List<JjimVO> jjimListData(String id)
  {
	  ...
	  {
		  conn=CreateConnection.getConnection();
		  String sql="SELECT /*+ INDEX_DESC(project_all_jjim pj_jno_pk)*/jno,no,"
				    +"(SELECT DISTINCT name FROM project_food WHERE fno=project_all_jjim.no),"
				    +"(SELECT DISTINCT poster FROM project_food WHERE fno=project_all_jjim.no),"
				    +"(SELECT DISTINCT address FROM project_food WHERE fno=project_all_jjim.no),"
				    +"(SELECT DISTINCT tel FROM project_food WHERE fno=project_all_jjim.no) "
