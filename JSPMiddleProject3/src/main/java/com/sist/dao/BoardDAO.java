...
public class BoardDAO {
	...
	public void boardReplyInsert(int pno, BoardVO vo)
	{
		try
		{
			...
			rs.close();
			// 2. group_step 조절 // 답변 게시판의 핵심
			sql = "UPDATE replyBoard SET "
					+ "group_step=group_step+1 "
					+ "WHERE group_id=? AND group_step>?"; // id가 같으면 step이 보다 큰 것들을 하나씩 증가
			ps=conn.prepareStatement(sql);
			ps.setInt(1, pvo.getGroup_id());
			ps.setInt(2, pvo.getGroup_step());
			ps.executeUpdate();
			// 3. insert
			sql = "INSERT INTO replyBoard(no,name,subject,content,pwd,regdate,hit,group_id,group_step,group_tab,root,depth) "
					+ "VALUES(rb_no_seq.nextval,?,?,?,?,SYSDATE,0,?,?,?,?,0)";
			ps=conn.prepareStatement(sql);
			ps.setString(1, vo.getName());
			ps.setString(2, vo.getSubject());
			ps.setString(3, vo.getContent());
			...
			ps.setInt(8, pno);
			ps.executeUpdate();
			// 4. depth 증가	// 삭제를 고려하여 만든 시스템 : depth가 0이면 그냥 삭제, depth가 존재하면 "관리자가 삭제한 게시글" 처리
			sql = "UPDATE replyBoard SET "
					+ "depth=depth+1 "
					+ "WHERE no=?";
			ps=conn.prepareStatement(sql);
